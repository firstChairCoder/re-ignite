...
const clear = (toolbox: GluegunToolbox) => {
  ...
  const spinner = print.spin(
    `Removing the dependency cache at '${cache.rootdir()}'`
  );
  cache.clear();
...
const help = (_toolbox: GluegunToolbox) => {
  ...
  p();
  p(
    "ignite cache is a parent command for interacting with the dependency cache"
  );
  p(
    "ignite uses on your machine to speed up the packager installation process."
  );
  p();
  heading("Subcommands:");
  p();
  ...
  command(
    { m: "path", width },
    "Return the path of the dependency cache on your computer",
    ["ignite cache path"]
  );
  p();
  command({ m: "clear", width }, "Clear the dependency cache", [
    "ignite cache clear"
  ]);
  igniteHeading();
};
