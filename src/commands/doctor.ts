...
module.exports = {
  ...
  run: async function (toolbox: GluegunToolbox) {
    ...
    const pnpmVersion =
      pnpmPath && (await run("pnpm --version", { trim: true }));
    ...
    const pnpmInfo = [
      column1("pnpm"),
      column2(pnpmVersion),
      column3(pnpmPath || "not installed")
    ];
    const bunInfo = [
      column1("bun"),
      column2(bunVersion),
      column3(bunPath || "not installed")
    ];

    async function packageInfo(packagerName: "npm" | "yarn" | "pnpm" | "bun") {
      return (await packager.list({ packagerName, global: true })).map(
        (nameAndVersion) => [
    ...
    const pnpmPackages = pnpmPath
      ? [
          [
            column1("  "),
            column2("<no pnpm global package info available>"),
            column3("")
          ]
        ]
      : [];
    ...
    try {
      expoVersion = (await run(expoVersionCmd))
        ?.match(/expo@(.*)/)
        ?.slice(-1)[0];
