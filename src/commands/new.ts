...
import type { GluegunToolbox } from "../types";
import {
  copyBoilerplate,
  isAndroidInstalled,
...
import type { PackagerName } from "../tools/packager";
...
module.exports = {
  run: async (toolbox: GluegunToolbox) => {
    // #region Toolbox
    const { print, filesystem, system, meta, parameters, strings, prompt } =
      toolbox;
    const { kebabCase } = strings;
    const { exists, path, removeAsync, copy, read, write, homedir } =
      filesystem;
    ...
    const { validateProjectName } =
      require("../tools/validations") as ValidationsExports;
    ...
    const defaultBundleIdentifier = `com.${strings
      .pascalCase(projectName)
      .toLowerCase()}`;
    let bundleIdentifier = useDefault(options.bundle)
    ...
    const { validateBundleIdentifier } =
      require("../tools/validations") as ValidationsExports;
    ...
    let overwrite = useDefault(options.overwrite)
      ? defaultOverwrite
      : boolFlag(options.overwrite);
    ...
    let workflow = useDefault(options.workflow)
      ? defaultWorkflow
      : options.workflow;
    if (workflow === undefined) {
        ...
        () => ({
          ...
          choices: [
            ...
            },
            {
              name: "Bare",
              message:
                "Bare - For advanced usage (still contains some Expo libraries)"
            }
          ],
          initial: "Expo",
          prefix
      ...
      if (useExpo) {
        const expoWorkflowTypeResponse = await prompt.ask<{
          workflow: "expo" | "cng";
        }>(() => ({
          type: "select",
          name: "workflow",
    ...
    const needsPrebuild =
      workflow === "prebuild" || workflow === "cng" || workflow === "manual";
    ...
    const defaultPackagerName = availablePackagers.includes("yarn")
      ? "yarn"
      : "npm";
    let packagerName = useDefault(options.packager)
      ? defaultPackagerName
      : options.packager;

    const validatePackagerName = (input: unknown): input is PackagerName =>
      typeof input === "string" &&
      ["npm", "yarn", "pnpm", "bun"].includes(input);

    if (
      packagerName !== undefined &&
      validatePackagerName(packagerName) === false
    ) {
      p();
      p(
        yellow(
    ...
    if (
      packagerName !== undefined &&
      availablePackagers.includes(packagerName) === false
    ) {
      p();
      p(
        yellow(
          `Error: selected "${packagerName}" but packager was not available on system`
        )
      );
      process.exit(1);
    }
    ...
    if (packagerName === undefined) {
      const initial = availablePackagers.findIndex(
        (p) => p === defaultPackagerName
      );
      const NOT_FOUND = -1;
      ...
      if (initial === NOT_FOUND) {
        ...
        p(
          yellow(
            `Error: Default packager "${defaultPackagerName}" was not available on system`
          )
        );
        process.exit(1);
      }

      const packagerNameResponse = await prompt.ask<{
    ...
    const boilerplate = (...pathParts: string[]) =>
      path(boilerplatePath, ...pathParts);
    ...
    let experimentalNewArch = useDefault(newArch)
      ? defaultNewArch
      : boolFlag(newArch);
    if (experimentalNewArch === undefined && workflow !== "expo") {
      const newArchResponse = await prompt.ask<{
        experimentalNewArch: boolean;
      }>(() => ({
        type: "confirm",
        name: "experimentalNewArch",
        message:
          "❗EXPERIMENTAL❗Would you like to enable the New Architecture?",
    ...
    const timeout = noTimeout
      ? undefined
      : setTimeout(timeoutExit, MAX_APP_CREATION_TIME);
    ...
    try {
      ...
      const logo =
        terminalWidth > 80
          ? () => ascii("logo.ascii.txt")
          : () => ascii("logo-sm.ascii.txt");
      p();
      p();
      p();
      p();
      ...
      p(
        ` █ Creating ${em(projectName)} using ${em(`Ignite ${igniteVersion}`)}`
      );
      p(
        ` █ Powered by ${ir(" ∞ Infinite Red ")} (${link(
          "https://infinite.red"
        )})`
      );
      ...
      if (exists(targetIgnorePath) === false) {
        warning(
          `  Unable to copy ${boilerplateIgnorePath} to ${targetIgnorePath}`
        );
      ...
      if (needsPrebuild) {
        ...
        if (expoVersion) {
          const expoDistTagOutput = await system.run(
      ...
      if (removeDemo) {
        log(
          `Removing demo dependencies... ${demoDependenciesToRemove.join(", ")}`
        );
      ...
      const boilerplatePackageJsonHash = cache.hash(
        read(path(boilerplatePath, "package.json"))
      );
      const cachePath = path(
        cache.rootdir(),
        boilerplatePackageJsonHash,
        packagerName
      );
      ...
      if (installDeps === true) {
        ...
        if (needsPrebuild) {
          ...
          await packager.run("prebuild:clean", {
            ...packagerOptions,
            onProgress: log
          });
      ...
      try {
        const IGNITE =
          "node " + filesystem.path(__dirname, "..", "..", "bin", "ignite");
      ...
      if (git === true) {
        ...
        try {
          ...
          if (isWindows) {
            ...
            await system.run(
              log(`git commit -m "New Ignite ${meta.version()} app`)
            );
          } else {
            await system.run(
              log(`
      ...
      const prettyCliCommand = cliCommand
        .split(" ")
        .map((c) =>
          c === projectName || c?.startsWith("--") ? `${c} \\${EOL}` : c
        ) // add a line break after the project name and each flag
        .map((c, i, a) => (i === a.length - 1 ? c.replace(`\\${EOL}`, "") : c)) // remove the line break after the last flag
        .map((c) =>
          c.startsWith("--") ? INDENT + CMD_INDENT + CMD_INDENT + c : c
        ) // add whitespace to the flags so it looks nice
        .join(" ");
      ...
      if (warnAboutEOL) {
        ...
        p2();
        p2(
          yellow(`Generator templates could not be converted to Windows EOL.`)
        );
        p2(
          yellow(
            `You may want to update these manually with your code editor, more info at:`
          )
        );
        p2(
          `${link(
            "https://github.com/infinitered/ignite/blob/master/docs/Generators.md"
          )}`
        );
        p2();
      }
      ...
      p2();
      p2(
        `Join our Slack community at ${link("http://community.infinite.red.")}`
      );
      p2();
...
function buildCliCommand(args: {
  ...
  const stringFlag = ([key, value]: FlagEntry) =>
    `--${kebabCase(key)}=${value}`;
  ...
  const cliCommand = `npx ignite-cli new ${projectName} ${(
    Object.entries(flags) as FlagEntry[]
  )
    .filter(([key]) => privateFlags.includes(key) === false)
    .filter(([, value]) => value !== undefined)
    .map(([key, value]) =>
      typeof value === "boolean"
        ? booleanFlag([key, value])
        : stringFlag([key, value])
    )
    .join(" ")}`;
