...
module.exports = {
  ...
  run: async (toolbox: GluegunToolbox) => {
    ...
    if (!filesystem.exists(appJsonPath)) {
      warning(
        "You must be in the root of a React Native project to rename it."
      );
    ...
    const oldBundleIdentifier =
      appJson.expo.android.package ?? appJson.expo.ios.bundleIdentifier;
    ...
    heading(
      `Ignite successfully renamed your app from ${red(oldName)} to ${green(
        newName
      )}!`
    );
    p();
    ...
    info(
      `      Carefully examine the diff it produces with \`git diff\` before committing.`
    );
    info(`    * Additionally, you'll want to re-run \`pod install\` and`);
    info(
      `      rebuild your app and caches. We don't make any changes to those files or folders.`
    );
    p();
    ...
    warning(
      `(${red(
        "careful"
      )}: this will remove all other changes you haven't committed!)`
    );
  }
};
