...
import type { GluegunPatchingPatchOptions, GluegunToolbox } from "gluegun";
import { filesystem, patching, strings } from "gluegun";
  ...
  generators.forEach((g) => {
    if (g === "app-icon") {
      ...
      command(g.padEnd(longestGen), `generates app-icons`, [
        `ignite g ${g} all|ios|android|expo`
      ]);
...
export async function generateFromTemplate(
  ...
  for (const templateFilename of files) {
    ...
    if (!content) {
      warning(
        "⚠️  Unable to parse front matter. Please check your delimiters."
      );
      return { written, exists, overwritten };
    }

    // where are we copying to?
    const generatorDir = path(
      appDir(),
      pluralize(generator),
...
export async function validateAppIconGenerator(
  ...
  const validationPromises = inputFileNames.map(async (fileName) => {
    const boilerplateInputFilePath = path(
      sourceDirectory(),
      "app-icon",
      fileName
    );
    ...
    const isSameAsSource = await (async function () {
      ...
      const sourceFileMd5 = inspect(boilerplateInputFilePath, {
        checksum: "md5"
      }).md5;
  ...
  const validationMessages = validationResults.reduce((acc, r) => {
    const messages = [
      ...
      r.isInvalidSize &&
        "   • the file is the wrong size (expected 1024x1024px)",
      r.isSameAsSource &&
  ...
  for (const o of options) {
    ...
    for (const i of icons) {
      ...
      for (const r of rules) {
        ...
        if (!inputFile) {
          warning(
            `⚠️  ${outputFileName}: transform failed, please file an issue on GitHub`
          );
          continue;
        }
        ...
        try {
          ...
          warning(
            `⚠️  ${outputFileName}: saving failed, check if the directory exists`
          );
        }
      }
    }
    ...
    const postGenerationFn = {
      ios: () => {
        ...
        copy(sourceContentsJsonFilePath, path(outputDirPath, "Contents.json"), {
          overwrite: true
        });
      ...
      expo: () => {
        ...
        const sourceExpoConfig = require(path(
          boilerplateDirPath,
          "app.json"
        ))?.expo;
        ...
        if (!exists(outputAppConfig)) {
          ...
          if (appConfigFiles.length > 0) {
            ...
            warning(
              `⚠️  Please add the following to your app.config.js manually:`
            );
            ...
            warning(
              `⚠️  No "app.json" found at "${outputAppConfig}". Skipping...`
            );
          }
...
export async function validateSplashScreenGenerator(
  ...
  if (messages.length) {
    validationMessages.push(
      `⚠️  ignite/templates/splash-screen/logo.png:`,
      ...messages
    );
  }
...
export async function generateSplashScreen(options: {
  ...
  async function generateForExpo(
    ...
    for (const expoRule of expoRules) {
      ...
      const outputFileName =
        [`splash-logo`, type, name].filter(Boolean).join("-") + ".png";
  ...
  if (isExpoOutputDirExists) {
    ...
    await generateForExpo("web", 300, [
      { width: 1920, height: 1080, scale: 1 }
    ]);
    await generateForExpo("all", 180, [
      { width: 1242, height: 2436, scale: 3 }
    ]);
    ...
    warning(
      `⚠️  No output directory found for "Expo" at "${expoOutputDirPath}". Skipping...`
    );
  }
