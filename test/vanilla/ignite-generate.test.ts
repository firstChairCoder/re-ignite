...
const setup = (): { TEMP_DIR: string } => {
  ...
  beforeEach(() => {
    ...
    filesystem.copy(BOILERPLATE_PATH + "/app", TEMP_DIR + "/app", {
      overwrite: true
    });
    filesystem.copy(BOILERPLATE_PATH + "/ignite", TEMP_DIR + "/ignite", {
      overwrite: true
    });
  });
...
describe("ignite-cli generate", () => {
   ...
   */
  const replaceHomeDir = (
    result: string,
    { mock = "/user/home/ignite", temp = TEMP_DIR } = {}
  ...
  describe("model", () => {
    it("should generate Pizza model and test, patch index model export, not patch RootStore and testing --overwrite option", async () => {
      ...
      expect(read(`${TEMP_DIR}/app/models/Pizza.ts`)).toMatchInlineSnapshot(`
        ...
        "
      `);
      expect(read(`${TEMP_DIR}/app/models/Pizza.test.ts`))
      ...
      const resultWithoutOverwriteOption = await runIgnite(
        `generate model Pizza`,
        options
      );
      expect(replaceHomeDir(resultWithoutOverwriteOption))
        .toMatchInlineSnapshot(`
        "   
      ...
      const resultWithOverwriteOption = await runIgnite(
        `generate model Pizza --overwrite`,
        options
      );
    ...
    it("should generate PizzaStore model and test, patch index model export, patch RootStore and testing --overwrite option", async () => {
      ...
      expect(replaceHomeDir(result)).toMatchInlineSnapshot(`
        ...
        "
      `);
      expect(read(`${TEMP_DIR}/app/models/PizzaStore.ts`))
        .toMatchInlineSnapshot(`
        "import { Instance, SnapshotIn, SnapshotOut, types } from \\"mobx-state-tree\\"
        import { withSetPropAction } from \\"./helpers/withSetPropAction\\"
        ...
        "
      `);
      expect(read(`${TEMP_DIR}/app/models/PizzaStore.test.ts`))
        .toMatchInlineSnapshot(`
      ...
      expect(read(`${TEMP_DIR}/app/models/index.ts`)).toMatchInlineSnapshot(`
        ...
        "
      `);
      expect(read(`${TEMP_DIR}/app/models/RootStore.ts`))
        .toMatchInlineSnapshot(`
  ...
  describe("components", () => {
    it("should generate Topping component and patch index components export", async () => {
      ...
      expect(replaceHomeDir(result)).toMatchInlineSnapshot(`
        ...
        "
      `);
      expect(read(`${TEMP_DIR}/app/components/Topping.tsx`))
        .toMatchInlineSnapshot(`
        ...
        "
      `);
      expect(read(`${TEMP_DIR}/app/components/index.ts`))
        .toMatchInlineSnapshot(`
    ...
    it("should generate Topping component in subdirectory and patch index components export", async () => {
      const result = await runIgnite(
        `generate component sub/to/my/Topping`,
        options
      );
      ...
      expect(replaceHomeDir(result)).toMatchInlineSnapshot(`
        ...
        "
      `);
      expect(read(`${TEMP_DIR}/app/components/sub/to/my/Topping.tsx`))
        .toMatchInlineSnapshot(`
        "import * as React from \\"react\\"
        import { StyleProp, TextStyle, View, ViewStyle } from \\"react-native\\"
        import { observer } from \\"mobx-react-lite\\"
