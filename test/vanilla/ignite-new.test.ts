...
import { run, runError, runIgnite } from "../_test-helpers";
  ...
  describe(`ignite new ${APP_NAME} --debug --packager=bun --yes`, () => {
    ...
    it("should have modified the package.json to have versions that work with expo go", () => {
      ...
      Object.keys(expoGoCompatExpectedVersions).forEach((key) => {
        expect(igniteJSON.dependencies[key]).toBe(
          expoGoCompatExpectedVersions[key]
        );
      });
    });
    ...
    it("should be able to use `generate` command and have pass output pass bun run test, bun run lint, and bun run compile scripts", async () => {
      ...
      // components
      const componentGen = await runIgnite(
        `generate component womp-bomp`,
        runOpts
      );
      expect(componentGen).toContain(`app/components/WompBomp.tsx`);
      expect(filesystem.list(`${appPath}/app/components`)).toContain(
        "WompBomp.tsx"
      );
      expect(
        filesystem.read(`${appPath}/app/components/WompBomp.tsx`)
      ).toContain("export const WompBomp");
      ...
      // screens
      const screenGen = await runIgnite(
        `generate screen bowser-screen --skip-index-file`,
        runOpts
      );
      expect(screenGen).toContain(`Stripping Screen from end of name`);
      expect(screenGen).toContain(`app/screens/BowserScreen.tsx`);
      expect(filesystem.list(`${appPath}/app/screens`)).toContain(
        "BowserScreen.tsx"
      );
      expect(
        filesystem.read(`${appPath}/app/screens/BowserScreen.tsx`)
      ).toContain("export const BowserScreen");
      ...
      const allAppIcons = filesystem.find(
        filesystem.path(appPath, iconSearchPath),
        {
          directories: false,
          files: true,
      ...
      const inputFiles = filesystem.find(
        `${appPath}/ignite/templates/app-icon`,
        {
          directories: false,
          files: true,
          matching: "*.png"
      ...
      const splashScreenAssets = filesystem.find(
        filesystem.path(appPath, splashSearchPath),
        {
          directories: false,
          files: true,
      ...
      function verifySplashScreenColor(
        type: "android" | "ios" | "expo",
        matchString: string
      ) {
        const splashScreenColorStrings = {
          ...
          ios: filesystem.read(
            filesystem.path(appPath, "ios/Foo/BootSplash.storyboard")
          ),
      ...
      const inputFile = filesystem.path(
        appPath,
        "ignite/templates/splash-screen/logo.png"
      );
      ...
      // commit the change
      await run(
        `git add ./app/models ./app/components ./app.json ./assets/images`,
        runOpts
      );
      ...
      expect(await run("git diff HEAD --no-ext-diff", runOpts)).toContain(
        "+  Bowser: undefined"
      );
      // #endregion
  ...
  describe(`ignite new ${APP_NAME} --debug --packager=bun --workflow=cng --yes`, () => {
    ...
    beforeAll(async () => {
      ...
      result = await runIgnite(
        `new ${APP_NAME} --debug --packager=bun --workflow=cng --yes`,
        {
          pre: `cd ${tempDir}`,
          post: `cd ${originalDir}`
        }
    ...
    it("should NOT have modified the package.json to have versions that work with expo go", () => {
      ...
      expect(igniteJSON).toHaveProperty("dependencies");
      Object.keys(expoGoCompatExpectedVersions).forEach((key) => {
        expect(igniteJSON.dependencies[key]).not.toBe(
          expoGoCompatExpectedVersions[key]
        );
      });
    });
